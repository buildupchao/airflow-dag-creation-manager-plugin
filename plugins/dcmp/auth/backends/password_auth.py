...
if __package__:
    ...
    class PasswordUser(models.User):
        ...
        def get_profile(self):
            ...
            return session.query(DcmpUserProfile).filter(DcmpUserProfile.user_id==self.user.id).first()

        def data_profiling(self):
            '''Provides access to data profiling tools'''
            profile = self.get_profile()
            if profile:
                return profile.is_data_profiler
            return False
        ...
        def is_approver(self):
            profile = self.get_profile()
            if profile:
                return profile.is_approver
            return False

        def get_approval_notification_emails_list(self):
            profile = self.get_profile()
            if profile:
                return profile.approval_notification_emails_list
            return []
